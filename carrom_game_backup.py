...
class CarromGame:
    ...
    def draw_board(self):
        ...
        # Draw inner border
        pygame.draw.rect(self.screen, theme["lines"], (board_margin + 20, board_margin + 20, 
                                             BOARD_SIZE - 40, BOARD_SIZE - 40), 2)
        
        # Draw diagonal lines connecting corners
        pygame.draw.line(self.screen, theme["lines"], 
                        (board_margin + 20, board_margin + 20), 
                        (board_margin + BOARD_SIZE - 20, board_margin + BOARD_SIZE - 20), 2)
        pygame.draw.line(self.screen, theme["lines"], 
                        (board_margin + BOARD_SIZE - 20, board_margin + 20), 
                        (board_margin + 20, board_margin + BOARD_SIZE - 20), 2)
        
        # Draw center circle (outer)
        pygame.draw.circle(self.screen, theme["accent"], (center_x, center_y), 70, 2)
        
        # Draw center circle (inner)
        pygame.draw.circle(self.screen, theme["lines"], (center_x, center_y), 50, 2)
        
        # Draw center star (simplified)
        for i in range(8):
            angle = i * math.pi / 4
            ...
            pygame.draw.line(self.screen, theme["lines"], (x1, y1), (x2, y2), 2)
        
        # Draw arrow circles in each quadrant
        quadrant_centers = [
        ...
        for qc in quadrant_centers:
            pygame.draw.circle(self.screen, theme["lines"], qc, 30, 1)
            
        # Draw pockets (red circles in corners)
        for pocket in self.pockets:
            pygame.draw.circle(self.screen, theme["pocket"], pocket, POCKET_RADIUS)
            pygame.draw.circle(self.screen, theme["lines"], pocket, POCKET_RADIUS, 2)
            
        # Draw baseline for current player
        if self.current_player == 1:
            pygame.draw.line(self.screen, theme["accent"], 
                            (board_margin + 50, SCREEN_HEIGHT - board_margin - 50), 
                            (SCREEN_WIDTH - board_margin - 50, SCREEN_HEIGHT - board_margin - 50), 2)
        else:
            pygame.draw.line(self.screen, theme["accent"], 
                            (board_margin + 50, board_margin + 50), 
                            (SCREEN_WIDTH - board_margin - 50, board_margin + 50), 2)
